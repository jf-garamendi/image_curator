...
def compute_roc(distances, matches, thresholds, fold_size=10):
    ...
    for fold_index, (training_indices, val_indices) \
        ...
        val_matches = val_matches.bool()
        for threshold_index, threshold in enumerate(thresholds):
            predicts = val_distances < threshold

            tp = torch.sum((predicts & val_matches).float()).item()
            fp = torch.sum((predicts & ~val_matches).float()).item()
            tn = torch.sum((~predicts & ~val_matches).float()).item()
            fn = torch.sum((~predicts & val_matches).float()).item()

            tpr[fold_index][threshold_index] = float(tp) / (tp + fn + 1e-8)
            fpr[fold_index][threshold_index] = float(fp) / (fp + tn + 1e-8)
