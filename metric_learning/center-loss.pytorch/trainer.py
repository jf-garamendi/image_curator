    ...
    def train(self):
        for self.current_epoch in range(self.start_epoch, self.max_epoch+1):
            ...
            if not (self.current_epoch % self.persist_stride):
                self.persist(is_best=False)
            
            if self.best_top1acc < self.validation_losses['top1acc'][-1]:
                self.persist(is_best=True)
                self.best_top1acc = self.validation_losses['top1acc'][-1]
    ...
    def run_epoch(self, mode):
        ...
        with torch.set_grad_enabled(mode == 'train'):
            for images, targets, names in dataloader:
                ...
                total_top3_matches += self._get_matches(targets, logits, 1)
